...
# 1) ROI registration, from Template space to DTI space, using ant batch
if [[ ${SWITCH[@]/_1_/} != ${SWITCH[@]} ]]; then
echo "$(date +%T)  ========= 1_ROI_registration_ants start! =========" |tee -a ${WD}/log/progress_check.txt
T=$(date +%s)
bash ${PIPELINE}/1_ROI_registration_ants.sh ${YoN} ${PIPELINE} ${WD} ${DATA_DIR} ${TemD} ${ROI} ${SUB_LIST} ${N_rf} ${M_ants} ${R_ants} ${syn_ants} ${i_ants} ${TEMPLATE} ${LEFT} ${RIGHT}
T=$(($(date +%s)-T))
echo "$(date +%T)  ========= 1_ROI_registration_ants done! =========" |tee -a ${WD}/log/progress_check.txt
printf "Time elapsed: %02d:%02d:%02d:%02d\n\n" "$((T/86400))" "$((T/3600%24))" "$((T/60%60))" "$((T%60))" |tee -a ${WD}/log/progress_check.txt
fi
...
echo "$(date +%T)  =========== 6_ROI_toMNI_ant start! ===========" |tee -a ${WD}/log/progress_check.txt
T="$(date +%s)"
bash ${PIPELINE}/6_ROI_toMNI_ant.sh ${PIPELINE} ${WD} ${ROI} ${SUB_LIST} ${MAX_CL_NUM} ${WTFD} ${TEMPLATE} ${VOX_SIZE} ${METHOD} ${LEFT} ${RIGHT}
T="$(($(date +%s)-T))"
echo "$(date +%T)  =========== 6_ROI_toMNI_ant done! ===========" |tee -a ${WD}/log/progress_check.txt
printf "Time elapsed: %02d:%02d:%02d:%02d\n\n" "$((T/86400))" "$((T/3600%24))" "$((T/60%60))" "$((T%60))" |tee -a ${WD}/log/progress_check.txt
fi
...
# 13) generate probabilistic tractography for each seed in brain
if [[ ${SWITCH[@]/_13_/} != ${SWITCH[@]} ]]; then
echo "$(date +%T)  =========== 13_Seed_probtrackx_mask start! ===========" |tee -a ${WD}/log/progress_check.txt
T="$(date +%s)"
bash ${PIPELINE}/13_Seed_probtrackx_mask.sh ${WD} ${DATA_DIR} ${ROI} ${SUB_LIST} ${N_SAMPLES} ${DIS_COR} ${LEN_STEP} ${N_STEPS} ${CUR_THRES} ${LEFT} ${RIGHT}
T="$(($(date +%s)-T))"
echo "$(date +%T)  =========== 13_Seed_probtrackx_mask done! ===========" |tee -a ${WD}/log/progress_check.txt
printf "Time elapsed: %02d:%02d:%02d:%02d\n\n" "$((T/86400))" "$((T/3600%24))" "$((T/60%60))" "$((T%60))" |tee -a ${WD}/log/progress_check.txt
fi
