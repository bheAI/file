...
# pipeline directory
PIPELINE=/DATA/249/bhe/test/MonkeyCBP_PMC

# Template directory which contains Template and ROI files, e.g., PMC_L.nii
TemD=/DATA/249/bhe/test/DATA/Template
...
#===============================================================================
# 1_ROI_registration_ants, whether to edit, 1--yes, 0--no 
YoN=1

# 1_ROI_registration_ANTs.sh, 6_ROI_to_TEM_ANTs.sh, template image
TEMPLATE=${TemD}/civm_rhesus_b0.nii.gz

# 1_ROI_registration_ants, name of Registration file
N_rf=b0

# 1_ROI_registration_ants, mode of the registration, CC/MI
M_ants=CC

# 1_ROI_registration_ants, radius, default CC=5; MI=32
R_ants=5

# 1_ROI_registration_ants, symmetric normalization
syn_ants=0.5

# 1_ROI_registration_ants, number of iterations, default 100
i_ants=300

# 2_ROI_calc_coord, NIFTI toolbox directory
NIFTI=${PIPELINE}/NIfTI_201903022
...
# 3_ROI_probtrackx, the length of each step, default 0.2 mm
LEN_STEP=0.2
...
# 6_ROI_toMNI_ant, new voxel size, default 1*1*1, please pay attention to the point, for example 0.1 or 0.30230 
VOX_SIZE=0.3

# 6_ROI_toMNI_ant, the number of the subject(delete this parameter is ok)
# SUBJECT_NUM=8
...
if command -v probtrackx2 > /dev/null 2>&1; then
	export COMMAND_PROBTRACKX=$(command -v probtrackx2)
else
	echo "Commmand 'probtrackx2' is not found! Please set it in config.sh!"
	exit 1
fi
